...
impl PluginLoader {
    ...
    fn discover_all_plugin_dirs() -> anyhow::Result<Vec<PathBuf>> {
        let mut dirs: Vec<PathBuf> = Vec::new();

        // 1. アプリ同梱の組み込みプラグイン(実行ファイル隣接)
        // if let Ok(exe_dir) = std::env::current_exe().and_then(|p| Ok(p.parent().unwrap().to_owned())) {
        //     let bundled = exe_dir.join("plugins");
        //     if bundled.exists() {
        //         dirs.extend(Self::discover_plugins(&bundled)?);
        //     }
        // }

        // 2. ユーザーディレクトリのプラグイン
        let user_plugins = pathes::plugins_dir()?;
    ...
    pub async fn load_from_disk(
        ...
        let dirs_again = Self::discover_all_plugin_dirs()?; // 簡略化のための重複呼び出し、下記コメント参照
        Ok(dirs_again
            .into_iter()
