...
Result<QVector<ClipPropertySchema>> getAllFields(ClientState *state, const Clip &clip) {
    ...
    return Result<QVector<ClipPropertySchema>>(
        raw, [](const FfiPropertySchema &schema) { return ClipPropertySchema(schema); });
}
...
Result<std::optional<ClipPropertyValue>> getValue(const Clip &clip, const QString &key) {
    ...
    return Result<std::optional<ClipPropertyValue>>(
        raw, [](const CClipBindingValue &value) { return ClipPropertyValue::fromC(value); });
}
