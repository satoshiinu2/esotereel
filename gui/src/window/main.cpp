...
MainWindow::MainWindow(ClientNetworkHandler &network, QWidget *parent) : QMainWindow(parent) {
    ...
    // タブの文字色をスタイルシートで設定
    this->dockManager->setStyleSheet("ads--CDockWidgetTab { color: palette(mid); font-size: 11px; }" // 非アクティブ（中間色）
    ...
    this->debugStreamsWidget = new DebugStreamsWidget();
    auto *debugStreamsDock = new ads::CDockWidget(this->dockManager, "DebugStreams");
