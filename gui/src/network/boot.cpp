...
void bootcore(QString corePath) {
    ...
    QAbstractSocket::connect(coreProcess, &QProcess::readyReadStandardOutput, [coreProcess]() {
        qDebug().noquote() << "[core stdout]" << coreProcess->readAllStandardOutput();
    });

    QAbstractSocket::connect(coreProcess, &QProcess::readyReadStandardError, [coreProcess]() {
